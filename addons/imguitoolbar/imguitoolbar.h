...
public:
    ...
    int render(bool allowdHoveringWhileMouseDragging=false,int limitNumberOfToolButtonsToDisplay=-1) const {
        ...
        if (!inWindowMode && (FLOAT_EQUAL(toolbarWindowSize.x, 0) || FLOAT_EQUAL(toolbarWindowSize.y,0))) updatePositionAndSize();
    ...
    inline void setScaling(float x,float y) {
        if (!FLOAT_EQUAL(scaling.x,x) || !FLOAT_EQUAL(scaling.y,y))   {
