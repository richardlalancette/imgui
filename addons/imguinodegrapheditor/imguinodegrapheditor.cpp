...
static void ImDrawListPathArcTo(ImDrawList *dl, const ImVec2 &centre, const ImVec2 &radii, float amin, float amax, int num_segments)  {
    if (!dl) return;
    if (radii.x == 0.0f || FLOAT_EQUAL(radii.y,0)) dl->_Path.push_back(centre);
...
inline static void GetVerticalGradientTopAndBottomColors(ImU32 c,float fillColorGradientDeltaIn0_05,ImU32& tc,ImU32& bc)  {
    if (FLOAT_EQUAL(fillColorGradientDeltaIn0_05,0)) {tc=bc=c;return;}
    ...
    if (cacheColorIn==c && FLOAT_EQUAL(cacheGradientIn,fillColorGradientDeltaIn0_05))   {tc=cacheTopColorOut;bc=cacheBottomColorOut;return;}
...
void NodeGraphEditor::render()
{
    ...
    if (show_left_pane) {
        ...
        const bool windowSizeChanged = (!FLOAT_EQUAL(lastWindowSize.x, windowSize.x) || !FLOAT_EQUAL(lastWindowSize.y, windowSize.y));
...
#       endif //NO_IMGUIHELPER_SERIALIZATION
    ...
    const Style& style = GetStyle();
	    ...
	    // Note that this change could break io.FontAllowUserScaling==true (To test, but it didn't work as expacted anyway)
            ...
            else if (!FLOAT_EQUAL(oldFontWindowScale,currentFontWindowScale)) {
        ...
        */
		    ...
		    if (show_node_copy_paste_buttons) ImGui::PopStyleVar(2);
                ...
                // BUTTONS ========================================================
                if (!FLOAT_EQUAL(node->Size.x,0) && !node->isInEditingMode)    {
...
bool FieldInfo::render(int nodeWidth)   {
    ...
    case FT_STRING: {
        ...
        if (flags<0) {
            ...
            const bool addBrowseButton = (!FLOAT_EQUAL(f.minValue,-500));
